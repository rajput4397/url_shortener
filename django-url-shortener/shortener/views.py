...
def check_url(request):
    if request.method == 'POST':
        try:
            ...
            if not original_url:  #if text is empty
                return JsonResponse({"error": "URL is required"}, status=400)
            
            if not validators.url(original_url):  # Using the validators library to check validity
            ...
            if url_entry:
                # URL already exists
                if timezone.now() < url_entry.expiration_time:  # if not  expired
                    return JsonResponse({"message": "URL already in use", "tiny_url": url_entry.tiny_url})
                url_entry.delete()   #if expired
